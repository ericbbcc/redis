...
//从指定DB中删除一个key,value,和关联的过期的记录。如果当前有足够的空间来分配需要的内存
//则释放对象的操作交给异步的线程来处理。
...
//异步的情况Redis DB,下面方法做的事情是创建一个新的哈希表,然后将旧的哈希表交给异步线程来处理。
...
// 清空Redis Cluster的slots-keys,这里可以看出,Redis Cluster的slots-keys的底层数据结构
// 是基于是基于跳跃表的数据结构
